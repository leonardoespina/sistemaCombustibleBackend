const { Solicitud, Usuario, Vehiculo, Dependencia, Subdependencia, PrecioCombustible, Moneda, Marca, Modelo, TipoCombustible } = require("../models");
...
exports.generarReporteDiario = async (req, res) => {
  try {
    ...
    const result = await paginate(Solicitud, req.query, {
      ...
      attributes: ['id_solicitud', 'codigo_ticket', 'fecha_validacion', 'cantidad_litros', 'cantidad_despachada', 'monto_total', 'precio_unitario', 'tipo_solicitud'],
    ...
    const formatVenta = (v) => {
      ...
      return {
        ...
        fecha: f,
        hora: f ? new Date(f).toLocaleTimeString('es-VE', { hour: '2-digit', minute: '2-digit', hour12: true }) : '',
    ...
    const reporteData = {
      institucional: institucionalData.map(i => {
        ...
        return {
          ...
          vehiculo: `${i.Vehiculo?.Marca?.nombre || ''} ${i.Vehiculo?.Modelo?.nombre || ''}`.trim(),
...
exports.consultarDespachos = async (req, res) => {
  try {
    ...
    const isValidFilter = (val) => val && val !== 'null' && val !== 'undefined' && val !== '';
    ...
    const result = await paginate(Solicitud, req.query, {
      ...
      attributes: ['id_solicitud', 'codigo_ticket', 'fecha_validacion', 'cantidad_litros', 'cantidad_despachada'],
    ...
    const filas = result.data.map(d => {
      ...
      return {
        ...
        fecha: f,
        hora: f ? new Date(f).toLocaleTimeString('es-VE', { hour: '2-digit', minute: '2-digit', hour12: true }) : '',
        vehiculo: `${d.Vehiculo?.Marca?.nombre || ''} ${d.Vehiculo?.Modelo?.nombre || ''}`.trim() || 'Desconocido',
