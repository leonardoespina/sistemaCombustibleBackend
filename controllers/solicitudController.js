...
exports.crearSolicitud = async (req, res) => {
  ...
  try {
    ...
    if (!placa) {
      ...
      return res
        .status(400)
        .json({ msg: "La placa del vehículo es requerida" });
    }
    ...
    if (solicitudActiva) {
      ...
      return res
        .status(400)
        .json({
          msg: `El vehículo ${placa} ya tiene una solicitud activa (Ticket: ${solicitudActiva.codigo_ticket || "Pendiente"}).`,
        });
    }
    ...
    console.log(
      "Cupo base encontrado:",
      cupoBase ? cupoBase.id_cupo_base : "NO ENCONTRADO",
    );
    ...
    if (!cupoBase) {
      ...
      return res
        .status(400)
        .json({
          msg: "No existe cupo base configurado para esta subdependencia y tipo de combustible.",
        });
    }
    ...
    if (!cupoActual) {
      ...
      console.log(
        `Cupo actual creado automáticamente para periodo ${periodoActual}, cupo_base ${cupoBase.id_cupo_base}`,
      );
    }
    ...
    if (tipo_solicitud === "VENTA") {
      ...
      if (!sub || !sub.cobra_venta) {
        ...
        return res
          .status(400)
          .json({
            msg: "Esta subdependencia no tiene habilitada la modalidad de venta.",
          });
      }
      ...
      if (id_precio) {
        ...
        if (precioObj) {
          ...
          monto_total = (parseFloat(cantidad_litros) * precio_unitario).toFixed(
            4,
          ); // Ajustado a 4 decimales según modelo
        }
      } else {
        await t.rollback();
        return res
          .status(400)
          .json({
            msg: "El ID de precio es obligatorio para solicitudes de tipo VENTA.",
          });
      }
    }
    ...
    if (!depObj) {
      ...
      return res
        .status(404)
        .json({ msg: "Dependencia no encontrada para generar ticket." });
    }
...
exports.aprobarSolicitud = async (req, res) => {
  ...
  try {
    ...
    if (solicitud.estado !== "PENDIENTE") {
      return res
        .status(400)
        .json({ msg: "La solicitud no está en estado Pendiente" });
    }
  ...
  try {
    ...
    if (["DESPACHADA", "VENCIDA", "ANULADA"].includes(solicitud.estado)) {
      ...
      return res
        .status(400)
        .json({
          msg: `No se puede rechazar una solicitud en estado ${solicitud.estado}`,
        });
    }
    ...
    if (cupo) {
      console.log(
        `Reintegrando ${solicitud.cantidad_litros} litros al cupo ID: ${cupo.id_cupo_actual} (${periodoOriginal}) por rechazo.`,
      );
