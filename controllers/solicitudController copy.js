...
exports.crearSolicitud = async (req, res) => {
  ...
  try {
    ...
    // Validar Estado del Cupo (No permitir si está INACTIVO o AGOTADO)
    if (cupoActual.estado === 'INACTIVO') {
      await t.rollback();
      return res.status(400).json({ 
        msg: "El cupo para esta subdependencia se encuentra INACTIVO. No se pueden generar solicitudes." 
      });
    }
    ...
    if (cupoActual.estado === 'AGOTADO') {
      await t.rollback();
      return res.status(400).json({ 
        msg: "El cupo para esta subdependencia se encuentra AGOTADO." 
      });
    }
